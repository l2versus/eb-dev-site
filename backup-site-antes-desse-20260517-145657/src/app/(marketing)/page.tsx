...
function NegativeHeroName() {
  ...
  useEffect(() => {
    ...
    if (!root || !reveal || !wave) return;

    const ctx = gsap.context(() => {
      gsap.set(root, {
        "--negative-reveal": "5%",
        "--negative-softness": "14%",
        "--negative-warp": "22px",
      });

      gsap.set(wave, {
        attr: {
          d: "M 0 125 C 210 96 410 154 620 122 C 820 92 1020 148 1240 112 L 1240 360 L 0 360 Z",
        },
      });

      const tl = gsap.timeline({
        scrollTrigger: {
          trigger: root.closest("#hero") ?? root,
          start: "top top",
          end: "bottom top",
          scrub: 1.15,
        },
      ...
      tl.to(
        root,
        {
          ...
          scale: 1.035,
          y: -16,
          filter: "url(#negativeHeroDistortion)",
          ease: "none",
        },
        0
      )
        ...
        .to(
          ...
          {
            attr: {
              d: "M 0 66 C 190 122 410 42 620 88 C 830 132 1010 52 1240 78 L 1240 360 L 0 360 Z",
            },
            ease: "none",
          },
          0
  ...
  const renderLine = (text: string) =>
    Array.from(text).map((char, index) => (
      <span
        key={`${text}-${char}-${index}`}
        className="negative-hero-name__char"
  ...
  return (
    <div ref={rootRef} className="negative-hero-name" aria-label="Emmanuel Bezerra">
      <svg className="negative-hero-name__svg" viewBox="0 0 1240 360" preserveAspectRatio="none" aria-hidden="true">
        <defs>
          <filter id="negativeHeroDistortion">
            <feTurbulence type="fractalNoise" baseFrequency="0.006 0.028" numOctaves="1" seed="7" result="noise" />
            <feDisplacementMap in="SourceGraphic" in2="noise" scale="1.35" xChannelSelector="R" yChannelSelector="G" />
          </filter>
        </defs>
        <path ref={waveRef} className="negative-hero-name__wave" />
      </svg>

      <div className="negative-hero-name__plate" />

      <h1 className="negative-hero-name__text negative-hero-name__text--base">
        <span className="negative-hero-name__line negative-hero-name__line--first">{renderLine("Emmanuel")}</span>
        <span className="negative-hero-name__line negative-hero-name__line--second">{renderLine("Bezerra")}</span>
      </h1>

      <div ref={revealRef} className="negative-hero-name__reveal" aria-hidden="true">
        <div className="negative-hero-name__paper" />
        <h1 className="negative-hero-name__text negative-hero-name__text--negative">
          <span className="negative-hero-name__line negative-hero-name__line--first">{renderLine("Emmanuel")}</span>
          <span className="negative-hero-name__line negative-hero-name__line--second">{renderLine("Bezerra")}</span>
        </h1>
      </div>
  ...
  return (
    <>
      ...
      <section
        id="hero"
        ref={heroRef}
        className="typographic-hero relative min-h-[120vh] overflow-hidden bg-[#050605]"
      >
        <motion.div
          style={{ y: heroY }}
        ...
        >
          <Image
            src="/images/ChatGPT Image 14 de mai. de 2026, 21_34_28.png"
            alt=""
            fill
            priority
            className="object-cover object-center opacity-55 blur-[2px] saturate-[0.7]"
            sizes="100vw"
          />
        </motion.div>
        <div className="typographic-hero__grade" aria-hidden="true" />
        <div className="typographic-hero__noise" aria-hidden="true" />
        <div className="typographic-hero__amber" aria-hidden="true" />

        <motion.div
          style={{ opacity: heroOpacity, scale: heroScale }}
          className="typographic-hero__stage"
        >
          <NegativeHeroName />
        </motion.div>

        <aside className="typographic-hero__stack" aria-label="Tecnologias">
          <span>React</span>
          <span>Node</span>
          <span>GSAP</span>
          <span>Lenis</span>
        ...
        <motion.div
          ...
          className="hidden absolute top-1/4 -left-48 w-[600px] h-[600px] bg-[#00f0ff] rounded-full blur-[200px]"
        />
        <motion.div
          animate={{ scale: [1.2, 1, 1.2], opacity: [0.03, 0.06, 0.03] }}
          transition={{ repeat: Infinity, duration: 10, ease: "easeInOut" }}
          className="hidden absolute bottom-0 -right-48 w-[500px] h-[500px] bg-[#ff00ff] rounded-full blur-[200px]"
        />
